...
//! Now ⇊⇊ we will write the " HTML CODE " for our header Section
  return (
    <div className="sticky top-0 z-10 flex flex-row  items-center justify-between h-14 px-4 md:px-5 bg-black">
      {loading && <Loader/>}       {/* //TODO: matlab k jab loading true ho then "Loader" component will execute here */}

      <div className="flex h-5 items-center">

        {pageName !== "video" && (   // TODO: matlab k agar pageName video k equal na ho to phir next hamari HTML run hogi
        ...
        <Link to="/" className='flex h-5 items-center'>
          ...
          <img className="h-full m-2 sm:hidden" src={ytlogoMobile} alt="Youtube" />
        </Link>
      </div>
        <div className="group flex items-center">
          <div className="flex h-8 md:h-10 md:ml-10 md:pl-5 border border-[#303030] rounded-l-3xl group-focus-within:border-blue-500 md:group-focus-within:ml-5 md:group-focus-within:pl-0">
            ...
            <input type='text' className="bg-transparent outline-none text-white px-5 md:pl-0 sm:w-42  sm:w-64 md:group-focus-within:pl-0 md:w-64 lg:w-[500px]" onChange={(e)=>setSearchQuery(e.target.value)} onKeyUp={searchQueryHandler} value={searchQuery}/>
          </div>
